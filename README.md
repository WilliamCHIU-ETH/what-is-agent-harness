...
**這是一張隨問題擴張的心智圖。** 先建立「模型與執行系統的分界」，再追問能力如何學來、工具如何接上、資訊如何留下。新內容會接回相關段落；也可以從文末的[問題地圖](#question-map)找到自己的疑問。
...
本文的 harness 指「讓模型持續執行任務的外部程式」這個職責範圍。它可以很小，也可以分布在本機與雲端的多個服務中。跨 session 的長期 memory、skills、多 agent 都是可加入的功能，不是每個 harness 的必備清單。
...
下載的權重由本機推論軟體載入並運行，也可以再包成 API，供其他程式呼叫。**API 是存取模型的介面，模型本身不是 API。** 本機也可以直接透過函式呼叫模型，不必經過 HTTP。[本機推論與服務化範例](https://qwenlm.github.io/blog/qwen3/)

所以，本機／雲端是部署位置；能否使用工具，則取決於模型能力，以及外部程式有沒有接上工具與執行流程。

<a id="function-calling"></a>

### Harness 是建立在 function calling 上嗎？

**更底層的基礎，是外部程式能接手模型的輸出，安排下一步。** API 原生的 function calling 提供一種結構化的工具請求介面；也可以約定文字格式，由程式解析，或讓模型產生程式碼，再交給執行環境。Hugging Face 的 [CodeAgent](https://huggingface.co/docs/smolagents/index) 就採用後者。

以文字模型為例，生成的 token 序列可以表達聊天回覆，也可以表達工具名稱與參數。外部軟體辨認這個約定格式後，才把它接到操作。因此，「能生成文字」和「能提出工具請求」可以同時成立。[工具請求與解析](https://huggingface.co/docs/transformers/chat_extras)

這裡先確定它在心智圖的位置：**function calling 是工具互動的介面；harness 還負責輸入、狀態與整個流程。**

<a id="tool-training"></a>

### 模型是不是已經被訓練成會用工具？

有些模型確實接受過這類訓練，但不能只從參數大小判斷。

| 訓練方式 | 模型練習什麼？ |
|---|---|
| 學習工具使用示範 | 根據任務與工具說明，選工具、填參數、理解結果 |
| 在環境裡進行多輪任務 | 操作工具、取得回饋、調整策略，再由訓練程序根據評估更新權重 |

例如 [Hermes 2 Pro](https://huggingface.co/NousResearch/Hermes-2-Pro-Mistral-7B) 加入了 function calling 訓練資料；[Qwen3-Coder](https://qwenlm.github.io/blog/qwen3-coder/) 則描述了在工具環境中進行多輪互動的強化學習。後者需要提供工具執行與回饋的系統，可以理解為訓練時也有 harness 類似的職責，但不一定和部署時用同一套程式。

**訓練留下的是「如何使用工具」的能力；部署時仍要提供可用工具、實際執行與權限。** 模型可能學過「改完程式應該測試」，下載權重卻不會自動取得你電腦的終端機。也可以根據當前提供的工具說明，嘗試使用訓練時未見過的工具，效果仍需驗證。

一般使用時，工具結果進入下一輪 context，影響下一次判斷；只有接上額外訓練程序，才會因此更新權重。
...
### Memory 是 harness 的必備核心嗎？

「Memory」有時指本次工作的歷史，有時指跨 session 的長期記憶，先問保存的是哪一種：

| 保存範圍 | 修 bug 情境裡的例子 | 扮演的角色 |
|---|---|---|
| 本次工作狀態 | 讀過哪些檔案、剛才測試失敗在哪裡 | 讓多輪任務能延續，通常需要以某種形式保留 |
| 跨 session 的長期記憶 | 下次仍記得這個專案使用哪套測試工具 | 減少重新交代背景，是可加入的能力 |

**沒有長期記憶，agent 仍能完成一次任務。** 而這兩種紀錄，都需要在適當時機放進輸入，才會影響模型。本文後面的表格用 memory 專指跨工作階段的保存。
...
你最容易看見工具調用和「它記得我」。其他部分也能從使用經驗找到：一直工作是 loop 在延續；要求確認是權限檢查；長對話仍能接續，可能涉及 context 整理。它們解決不同問題，再由程式串成執行系統。

| 隨理解增加而出現的問題 | 接回心智圖的位置 |
|---|---|
| 本機下載的模型，和雲端 API 是什麼關係？ | [模型與部署介面](#local-model) |
| Harness 一定需要原生 function calling 嗎？ | [輸出如何接成動作](#function-calling) |
| 模型原本就會用工具嗎？訓練時也有 harness？ | [學到的能力與執行環境](#tool-training) |
| 我只說一次，為什麼它會一直做？ | [Loop 與停止控制](#loop) |
| 模型不聽指令時，程式怎麼擋？ | [執行前檢查與環境權限](#control) |
| Memory 是必要的嗎？和 context、cache 有什麼不同？ | [保存範圍](#memory-scope)與[本輪輸入](#context) |

後續遇到新名詞，先問它在解決哪個問題、介入哪個位置，再把它接進來。
